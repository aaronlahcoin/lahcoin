...
// Copyright (c) 2011-2012 LahCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LahCoinTEST0\r");
            Send(hSocket, "WHO #LahCoinTEST0\r");
        } else {
            // randomly join #LahCoin00-#LahCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #LahCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LahCoin%02d\r", channel_number).c_str());
        }
