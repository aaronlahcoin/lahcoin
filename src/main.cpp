...
uint256 hashGenesisBlock("0xc880400951d8c220af511762e03ae7665822b21475966831a6409c8fbe29f7ad");
...
const string strMessageMagic = "LahCoin Signed Message:\n";
...
{
    // LahCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // LahCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "lahcoin genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x4d595aa95ff16c51bc6283327326bcd55621e66716a2c757b5cfb3139f0fbe1b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x32, 0xcf, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LahCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LahCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LahCoinMiner(CWallet *pwallet)
{
    printf("LahCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("LahCoin-miner");
    ...
    try { loop {
        ...
        printf("Running LahCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LahCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LahCoinMiner, pwallet));
}
